...
def get_ma_order_series(df, ma_mode="full"):
    ...
    -------
    order
        "long"（上昇並び） | "short"（下降並び） | "mixed"（どちらでもない）
        を各日について持つSeries
    """
...
def is_long_trend_series(df, slope_lookback=1, ma_mode="full"):
    ...
    for column in _trend_columns(ma_mode):
...
def is_short_trend_series(df, slope_lookback=1, ma_mode="full"):
...
def get_ma_order(df, ma_mode="full"):
    ...
    return get_ma_order_series(df, ma_mode=ma_mode).iloc[-1]
...
def is_long_trend(df, slope_lookback=1, ma_mode="full"):

    """
    直近日のロング（上昇トレンド）判定
    """

    return bool(is_long_trend_series(df, slope_lookback=slope_lookback, ma_mode=ma_mode).iloc[-1])


def is_short_trend(df, slope_lookback=1, ma_mode="full"):

    """
    直近日のショート（下降トレンド）判定
    """

    return bool(is_short_trend_series(df, slope_lookback=slope_lookback, ma_mode=ma_mode).iloc[-1])


def get_current_trend_period(df, direction, slope_lookback=1, ma_mode="full"):
    ...
    ----------
    df
        date, sma5, sma20（ma_mode="full"ならさらにsma60）列を持つ
    ...
    if direction == "long":
        trend = is_long_trend_series(df, slope_lookback=slope_lookback, ma_mode=ma_mode)
    elif direction == "short":
        trend = is_short_trend_series(df, slope_lookback=slope_lookback, ma_mode=ma_mode)
    else:
